...
PlatformIO extra_scripts pre: hook, the same way native_static.py already
...
try:
    ...
    # Same workaround, and same reason, as bally_dongle uses.
...
# Exit ONLY on failure, and never on success. SCons exec()s an extra_script in its own process, so a
