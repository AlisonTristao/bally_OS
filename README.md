...
- **ManifestResponder**: Responde `CONTROL/MANIFEST_REQUEST` descrevendo os schemas estáticos de `TelemetryPublisher` (`protocol.test`, `robot.state`, `system.monitor`) como `MANIFEST_DATA`; `config_revision` é uma constante (1), já que o catálogo deste firmware não muda em tempo de execução.
...
- **TelemetryPublisher**: Fila SPSC estática e não bloqueante para as amostras numéricas (`protocol.test` em `PACKED_LE` a até 50 Hz, `robot.state` nas transições); o documento `system.monitor` em `UTF8` (a até 0,33 Hz) é grande e de baixa taxa, então usa um slot de staging próprio, fora da fila. Todos preservam o timestamp da coleta.
