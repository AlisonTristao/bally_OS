...
8. **Antes de considerar pronto**, rode `pio run -e esp32-s3`. Se a lib só é usada atrás de uma flag (ex: algo como `#ifdef ENABLE_SYSTEM_MONITOR`), compile também com essa flag ligada antes de reverter — código atrás de uma flag desligada por padrão só é testado assim.
...
- **Antes de reduzir uma stack**, meça o *high water mark* de verdade (via `sysmon tasks`, com `ENABLE_SYSTEM_MONITOR`) em vez de chutar.
